import { createUuid, USER_DATA_DIR_NAME } from "@zcode/shared";
...
function resolveDeviceStateFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, USER_DATA_DIR_NAME, "v2", "telemetry-state.json");
  }
...
function resolveDeviceStateLockFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, USER_DATA_DIR_NAME, "v2", "telemetry-state.lock");
  }
