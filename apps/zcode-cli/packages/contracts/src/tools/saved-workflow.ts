...
/**
 * 全局作用域的存放目录（相对 agent 进程的家目录）。落点 `~/{USER_DATA_DIR_NAME}/workflows/<name>.dwf.ts`
 * ——与 legacy Workflow 工具的用户根同一处，对所有项目可见。
 */
export const SAVED_WORKFLOW_GLOBAL_DIR = `${USER_DATA_DIR_NAME}/workflows`;
 ...
 * 落在 `~/{USER_DATA_DIR_NAME}/workflows/`（agent 进程的家目录），对所有项目可见。一个文件的作用域由它所在的目录推得，frontmatter 不存。
 */
