...
Plugin state lives under `~/.reactor/cli/plugins`:
...
zcode reads hooks from the same main JSON config file as MCP, usually `~/.reactor/cli/config.json`. Hooks are disabled by default; set `hooks.enabled` to `true` and add process hooks under `hooks.events`.
