...
Plugin state lives under `~/.reactor-ds/cli/plugins`:
...
zcode reads hooks from the same main JSON config file as MCP, usually `~/.reactor-ds/cli/config.json`. Hooks are disabled by default; set `hooks.enabled` to `true` and add process hooks under `hooks.events`.
